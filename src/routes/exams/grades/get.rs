...
async fn get_all(req_body: String) -> Result<impl Responder, Error> {
    ...
    exam_id
        .exam()
        .await
        .map_err(|_| actix_web::error::ErrorNotFound("Exam by this id not found."))?;
    // getting all grades by exam_id
    let grades = exam_id.grades_by_exam().await.map_err(|_| {
        actix_web::error::ErrorInternalServerError(
            "Something went wrong while getting grades of this exam.",
        )
    })?;
